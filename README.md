...
However, trading in Forex is not all rose-colored, it does involve certain risks. We tend to make our buying and selling decisions based on uncertain predictions. Fear of loss and greed for win are the most important factors that influence the actions within a Forex market. To be a successful trader, one should be able to analyze and assess the situation in the market, and make decisions based on the analysis, not his/her emotions. When a trader enters a market, he/she has no idea when the prices (“foreign exchange rate” [2]) will rise, and when they will drop down. “Prices on exchanges are formed on the basis of the supply and demand. If more traders want to buy something (demand) than sell it (supply), then the price moves up (Uptrend). Conversely, if more traders want to sell something (supply) than buy it (demand), then the price moves down (Downtrend). So, the price is said to be floating, which provides a good opportunity for speculation on exchanges” [1]. Because there is no specific formula or rule to decide when is the right time to enter or exit a trade, traders have been striving to come up with different approaches to get as close as possible to make successful decisions. In this article, we are going to make our own entry and exit criteria depending on the pattern of prices we’re following. To find the pattern visually, we use candlestick charts.
...
METHODOLOGY:
  1.	We will begin by getting the data that we will be using on this research paper. We use an excel spreadsheet [5] that includes the following data:
  ...
  2.	In the original excel spreadsheet we generate binary values 0’s and 1’s by inputting the formula “=if(E3>E2,1,if(E3<E2,0,H2))” for Direction of Low (DL column). Then we use the formula “=if(F3>F2,1,if(F3<F2,0,I2))” for Direction of High (DH column).

  3.	We save our spreadsheet as .csv, which denotes “comma separated values” to simplify the read operations of our program.

  4.	We create an Algorithm coded using Java programming language (See Appendix) to analyze the data.
...
The goal is to buy a currency at a lower price and then sell it at a higher price. Once we find a match, we buy (enter the trade) at the open of the next (fifth) candle, then we calculate our limit of gain or loss by subtracting the low of the pivot (third) candle from the open of the fifth candle. Then we sell (exit the trade) once we reach either the gain or loss limit.

  8.	Look for the short Pattern: [11 11 11 00]
The goal is to sell a currency at a higher price and then buy it at a lower price. Once we find a match, we sell (enter the trade) at the open of the next (fifth) candle, then we calculate our limit of gain or loss by subtracting the open of the fifth candle from the high of the pivot (third) candle. Then we buy (exit the trade) once we reach either the gain or loss limit.

  9.	If we reach the gain limit we increase the number of wins and add the value of limit to the total gain in pips (see Appendix)... Else if we reach the loss limit we increase the number of losses and subtract the value of limit from the total loss in pips.
...
DISCUSSION OF RESULTS:
  For the long reversal pattern, we received positive results. The number of winning trades outweighed the number of losing trades, and we were able to make a fair amount of profit (pips). For example, we found 62% of winning trades, whereas 37% of losing trades in GBP/USD currency pair. 
For the short reversal pattern, the results were more impressive, and we found almost twice more matches than the long reversal pattern. The difference between the number of winning trades and the number of losing trades was very close compared to the result of long reversal pattern, but the amount of profit were more than that of a long reversal pattern. For instance, we found 67% of winning trades, whereas 32% of losing trades in GBP/USD currency pair. In terms of total gain, and average gain per trade, the results of short reversal pattern are more impressive than the long reversal patterns.
  
  While trying to get optimal results, we were experiencing many different difficulties. First, we had to deal with the dilemma of filling our arrays from the .csv file, as well as finding the patterns (Long and short). We had long group discussions debating and arguing about the logic implemented in the code. We made several changes to improve and strengthen the algorithm and make it as simple as possible for the beginners to understand. After long hours of failed attempts, finally we managed to overcome those difficulties.
...
CONCLUSION:
As mentioned earlier, investing in the Forex market (or any market) can be very risky. After researching and creating the program (see Code), we believe that our methods can help beginners better understand and identify the patterns, and make successful trades. The results we acquired show that future prices are influenced by past ones. Being able to effectively locate reversal patterns is what separates the winners from the losers, in terms of trading. 
